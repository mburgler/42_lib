...
/*   Updated: 2022/12/25 20:31:01 by mburgler         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*read_line(int fd, char *mem)
{
	...
	chars_read = 1;
	extracted = malloc(BUFFER_SIZE + 1);
	if (!extracted)
		return (NULL);
	while (chars_read > 0)
	{
...
char	*save_after_backslash_n(char *mem)
{
	...
	saved_leftover = malloc(sizeof(char) * (ft_strlen2(mem) - end_of_line));
	if (!saved_leftover)
	{
		free(mem);
		return (NULL);
	...
	while (mem[end_of_line])
		saved_leftover[j++] = mem[end_of_line++];
	saved_leftover[j] = '\0';
	free(mem);
	return (saved_leftover);
}
